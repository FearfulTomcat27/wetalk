...
func (h *FriendHandler) Add(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, types.ErrInvalidParam) {
			common.Error(c, http.StatusBadRequest, "不能添加自己为好友")
			return
		}
		if errors.Is(err, types.ErrConflict) {
			common.Error(c, http.StatusConflict, "好友请求已存在")
			return
		}
...
func (h *FriendHandler) Accept(c *gin.Context) {
	...
	if err != nil {
		common.Error(c, http.StatusBadRequest, "无效的请求 ID")
		return
	}

	if err := h.svc.AcceptFriend(userID, friendID); err != nil {
		if errors.Is(err, types.ErrNotFound) {
			common.Error(c, http.StatusNotFound, "好友请求不存在")
			return
		}
		if errors.Is(err, types.ErrUnauthorized) {
			common.Error(c, http.StatusForbidden, "无权操作")
			return
		}
		if errors.Is(err, types.ErrConflict) {
			common.Error(c, http.StatusConflict, "请求已处理")
			return
		}
...
func (h *FriendHandler) Delete(c *gin.Context) {
	...
	if err := h.svc.DeleteFriend(userID, friendID); err != nil {
		if errors.Is(err, types.ErrNotFound) {
			common.Error(c, http.StatusNotFound, "好友关系不存在")
			return
		}
		if errors.Is(err, types.ErrUnauthorized) {
			common.Error(c, http.StatusForbidden, "无权操作")
