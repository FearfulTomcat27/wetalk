...
func NewChatService(chatRepo ChatRepository, cache Cache) *ChatService {
	return &ChatService{
		chatRepo: chatRepo,
		cache:    cache,
	}
...
func (s *ChatService) EnsureSingleChat(user1ID, user2ID int64) (*model.Chat, error) {
	...
	// 先查找已有聊天
	chat, err := s.chatRepo.FindSingleChatByUserIDs(user1ID, user2ID)
	if err != nil {
		return nil, err
	}
	if chat != nil {
	...
	// 不存在则创建
	return s.chatRepo.CreateSingleChat(user1ID, user2ID)
}
...
func (s *ChatService) GetMemberIDs(chatID int64) ([]int64, error) {
	...
	var ids []int64
	if ok, _ := s.cache.Get(key, &ids); ok {
		return ids, nil
	}

	ids, err := s.chatRepo.GetMemberIDs(chatID)
	if err != nil {
		return nil, err
	}

	if cacheErr := s.cache.Set(key, ids, 1*time.Hour); cacheErr != nil {
		_ = cacheErr
	}
	return ids, nil
}
...
func (s *ChatService) UpdateLastMessage(chatID, messageID int64, content string, contentType string, msgTime time.Time) error {
	return s.chatRepo.UpdateLastMessage(chatID, messageID, content, contentType, msgTime)
}
